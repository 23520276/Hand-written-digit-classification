...
def huan_luyen_mo_hinh(dac_trung_sinh_vien, duong_dan_diem_ck):
    try:
        logger.info("Đọc file điểm CK...")
        diem_ck = pd.read_csv(duong_dan_diem_ck)
        logger.info(f"Các cột trong file ck-public.csv: {diem_ck.columns.tolist()}")

        if 'CK' in diem_ck.columns:
            diem_ck = diem_ck.rename(columns={'CK': 'diem_ck'})
        else:
            logger.error("Không tìm thấy cột 'CK' trong dữ liệu.")
            return None, None, None

        logger.info("Ghép đặc trưng với điểm CK...")
        ...
        y = pd.to_numeric(du_lieu_day['diem_ck'], errors='coerce')
...
def du_doan_diem(mo_hinh, scaler, features, dac_trung_sinh_vien):
    try:
        ...
        ket_qua = pd.DataFrame({
            ...
            'DiemCuoiKy_du_doan': diem_du_doan
        })
...
def main():
    try:
        logger.info("Bắt đầu quy trình dự đoán điểm CK...")
        dac_trung = tai_va_tien_xu_li_du_lieu()
        if dac_trung is None:
            return

        duong_dan_ck = os.path.join(DUONG_DAN_DU_LIEU, "ck-public.csv")
        mo_hinh, scaler, features = huan_luyen_mo_hinh(dac_trung, duong_dan_ck)
        ...
        if ket_qua is not None:
            duong_dan_ket_qua = os.path.join(DUONG_DAN_DU_LIEU, "ket_qua_du_doan_ck.csv")
